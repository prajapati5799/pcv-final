<?php
if( isset($_POST['district_csv_download']) )
{
	...
	$stateFields 	= get_district_form_fields();
	...
	$user_pre_qry    = 'SELECT DISTINCT ID FROM '. USER_TBL . ' as u LEFT JOIN '. USERMETA_TBL . ' as u1 ON u1.user_id = u.ID LEFT JOIN '. USERMETA_TBL . ' as u2 ON u2.user_id = u.ID WHERE ' . $usermeta_query . ' ORDER BY Id DESC';

	//$user_pre_sql   = $wpdb->prepare($user_pre_qry);
	$users          = $wpdb->get_results($user_pre_qry, ARRAY_A);
	$user_ids       = [];
	...
	if($user_ids) {
	    ...
	    if( $results && !empty($results) ){
	    	...
	    	$csv_value_data .= "District,".implode(',', $csv_key_value_k)."\n";
	    	...
	    	foreach ($results as $result) {
	        	...
	        	if(!empty($form_values)){
					...
					$csv_value_data .= get_user_city($result['user_id']).",".implode(',', $csv_key_value_v);
					$csv_value_data .= "  \n";
	        	}
			}
	    }
