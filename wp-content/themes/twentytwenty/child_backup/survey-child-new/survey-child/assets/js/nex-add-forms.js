(function ( $ ) {
	...
	function inputStoreSet(cname, cvalue, type) {
        ...
        if( typeof cvalue !== "undefined" ){
	        ...
	        for (var key in valArrayList){
		    	if( websafeUnicode(valArrayList[key]['name']) == cname ){
	...
	function store(name, type, dom) {
		...
		else 
		{
		  	...
		  	//console.log( name  + " => " + $(dom).attr('value') + " =>" + dom.value);
		  	
		  	if( typeof get_area_val === "undefined" || get_area_val.length == 0 ){
		  		//get_area_val = $(dom).attr('value');
    ...
    function setArrayListVariableValues(){
    	$.ajax({
		    ...
		    data: {action: "save_nex_form_steps_data",nex_form_id: $('input[name="nex_forms_Id"]').val(),data: valArrayList},
    ...
    function setValueInInput(){
    	...
    	for (var data_key in valArrayList){
    		...
    		var previousSet = valArrayList[data_key]['val'],
    			...
    			fname = websafeUnicode(valArrayList[data_key]['name']);
    		...
    		if((ftype && ftype == "select")){
				...
				// 	//the_super_check.parent().parent().parent().trigger('click');
					
				// 	the_super_check.parent().addClass('icon-checked');
				// 	//the_super_check.parent().parent().parent().addClass('radio_selected');
				...
				//var the_radio = $('input[name="'+fname+'"]').parent().find(':radio[name="'+fname+'"][value="'+ previousSet +'"]');
				/*setTimeout(function(){ the_radio.closest('label').trigger('click'); }, 100); 
				the_radio.closest('.icon-holder').trigger('click');*/


				//var the_super_check = $('input[name="'+fname+'"]').closest('.icon-container').find('input[value="'+ previousSet +'"]');
    ...
    function removeArrayListVariable(){
		valArrayList = [];
    };
	...
	jQuery(window).on('load', function()
	{
		...
		$.ajax({
		    ...
		    dataType: "json",
		    url: theme_js_vars.ajax_url,
		    data: {action: "send_saved_nex_form_data",nex_form_id: $('input[name="nex_forms_Id"]').val()},
		    success: function(response){
		        if(response.is_reflact){
		        	...
		        	if( response.data == null ){
		        		...
		        		for (var key in responseData){
					    	responseData[key]['name'] = websafeUnicode(responseData[key]['name']);
					    }

		        		valArrayList = responseData;*/
		        		valArrayList = response.data;
		        	}
		        	
		        	setValueInInput();
		        } else {
		        	findFormInput(autoPopulateNexForm);
	...
	$(document).on( 'click', ".nex-submit",  function(){
		if ( confirm('Are you sure want to submit application?') ){
	        ...
	        $.ajax({
			    ...
			    dataType: "json",
			    url: theme_js_vars.ajax_url,
			    data: {action: "published_saved_nex_form_data",nex_form_id: $('input[name="nex_forms_Id"]').val(),data: valArrayList},
			    success: function(response){},
				error: function(xhr, status, error){
					alert("Error!" + xhr.status);
				}
	...
	$(document).on( 'keyup', '#contact_details', function() {
	//$('#contact_details').on( 'keyup', function() {
        var maxWords = $(this).val();
     	var count = getWordCount( maxWords );
        if( count <= 50 ) {
            return true;
        } else {
            var limited_strings = getWordFromString(maxWords, 10);
            $(this).val(limited_strings);
        }
	});
