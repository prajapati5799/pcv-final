    ...
    public static function register_settings( $settings_class ) {
        ...
        $ACTIONS_SECTION->add_group( __( 'After-Login actions', 'ajax-login-and-registration-modal-popup' ), 'login', false, true )
            ...
            ->add_field( array(
                'slug'        => 'redirect',
                'name'        =>
	                __('Redirect to (if "Redirect to page" is selected)', 'ajax-login-and-registration-modal-popup') .
        ...
        ->description('Actions with a label will work only with a PRO version installed.');
        ...
        $ACTIONS_SECTION->add_group( __( 'After-Registration actions', 'ajax-login-and-registration-modal-popup' ), 'registration', false, true )
            ->add_field( array(
                ...
                'addons'      => array(
                    'options'     => array(
                        ...
                        'redirect' => 'Redirect to a page and auto-login',
                        'email-verification' => 'Email verification (send password to the email)',
                        'email-verification-pro' => 'Email verification (send a verify link)',
                        'email-verification-pro-w-redirect' => 'Email verification + redirect to page below (send a verify link)',
                    ),
                ),
                'default'     => 'none',
                'description' => __('"Email verification (send password to the email)" is not effective if the user can set password during registration', 'ajax-login-and-registration-modal-popup' ),
                'render'      => array( new LRM_Field_Select_W_PRO(), 'input' ),
                'sanitize'    => array( new LRM_Field_Select_W_PRO(), 'sanitize' ),
            ) )
            ->add_field( array(
            ...
            ->add_field( array(
                'slug'        => 'redirect',
                'name'        => __('Redirect to (only if "Redirect to a page and auto-login" or "Email verification + redirect" selected)', 'ajax-login-and-registration-modal-popup'),
                'addons'      => array(
                    'per_role' => false,
                ),
                'default'     => [],
            ...
            ->add_field( array(
                ...
                'addons'      => array(
                    'options'     => array(
                        ...
                        'redirect' => 'Redirect to the custom page',
                    ),
                ),
                'default'     => 'none',
                //'description' => __('Select an action', 'ajax-login-and-registration-modal-popup' ),
