...
function NEXForms_ui_output( $atts , $echo='',$prefill_array=''){
	...
	$custom_user_func_after_submit_js = 'function nf_custom_user_func_after_submit(formData, jqForm, options){';
			...
			$output .= '<div class="set_form_theme theme-'.$set_theme.'">';
				...
				/*if($form_attr->on_screen_confirmation_message_admin)
					...
					{*/
						...
						$get_success_message = wp_unslash($form_attr->on_screen_confirmation_message);

						if(class_exists('NEXForms_Shortcode_Processor'))
						{
							$shorcode_processor = new NEXForms_Shortcode_Processor();
							$get_success_message = $shorcode_processor->process_shortcodes($get_success_message);
						}
						...
						$osmsgv2 .= '<div class="nex_success_message msg_'.$msg_position.' msgv2 animated" style="display:none;">';
							$osmsgv2 .= '<div class="nex_success_message_container" style="'.$msg_css_style.'">';
								$osmsgv2 .= '<div class="msg_text">'.$get_success_message.'</div>';
							$osmsgv2 .= '</div>';
						$osmsgv2 .= '</div>';
						...
						if($form_attr->on_screen_confirmation_message_admin)
							...
							$get_success_message = str_replace('\\','',wpautop($form_attr->on_screen_confirmation_message));
							...
							$output .= '<div class="nex_success_message oldv animated" style="display:none;">';
								...
								$output .= '<div class="msg_box">';
									$output .= '<div class="msg_text" style="display:none;">'.$get_success_message.'</div>';
								$output .= '</div>';	
							$output .= '</div>';
							}
...
/*******************************************************************************************************/
	...
	// Delete if previous record available
	$form_user_id = filter_var($_REQUEST['user_id'],FILTER_SANITIZE_NUMBER_INT);
	$form_nex_id = filter_var($_REQUEST['nex_forms_Id'],FILTER_SANITIZE_NUMBER_INT);

	$wpdb->query("DELETE FROM " . $wpdb->prefix . "wap_nex_forms_entries WHERE `user_Id` = '" . $form_user_id . "' AND nex_forms_Id = '" . $form_nex_id . "' AND publish = '1'");
	...
	if( function_exists('final_nex_form_data_published_temp_tbl') ){
		$final_arr = [];
		$final_arr['file_data'] = $insert_file_array;
		$final_arr['record_id'] = $entry_id;
		$final_arr['user_id'] = $form_user_id;
		$final_arr['nex_form_id'] = $form_nex_id;
		$final_arr['files_val'] = $files_val;
		final_nex_form_data_published_temp_tbl($final_arr);
	}
...
/**************************************************/
	...
	$download_url = get_the_permalink(PDF_DISTRICT_FORM);
	if( $nex_forms_id == STATE_FORM_ID ){
		$download_url = get_the_permalink(PDF_STATE_FORM);
	}
	
	wp_send_json([
		'nex_forms_id' => $nex_forms_id, 
		'entry_id' => $entry_id, 
		'user_id' => filter_var($_POST['user_id'],FILTER_SANITIZE_NUMBER_INT), 
		'errors' => $error,
		'download_url' => $download_url . "?record_id=" . $entry_id . "&nex_form_id=" . $nex_forms_id . "&user_id=" . filter_var($_POST['user_id'],FILTER_SANITIZE_NUMBER_INT)
	]);
	die();
}
	  ...
	  $file_uploads->checkout = $dashboard->checkout;
	  	...
	  	$output .= '<div class="nex_forms_admin_page_wrapper ">';
			...
			$output .= '</div>';
		  	...
		  	//DASHBOARD
		  	$output .= '<div id="dashboard_panel" class="dashboard_panel form-tbl-list">';
			  ...
			  $output .= '</div>';
			  	...
			  	// ======================================================
			  	// == Start of "Submission Records"
			  	// == Added By Milan Hirapra
			  	// == 04-01-2121
			  	// ======================================================
			  	$output .= '<div class="row row_zero_margin ">';
					$output .= '<div class="col-sm-12">';
						
						$output .= '<div class="dashboard-box submitssion_reports">';
						   	
						   	$output .= '<div class="dashboard-box-header">';
						      	$output .= '<div class="table_title">';
						      		$output .= '<i class="material-icons header-icon">insert_drive_file</i>';
						      		$output .= '<span class="header_text">'.__('Submission Records','nex-forms').'</span>';
						      	$output .= '</div>';
						   	$output .= '</div>';
						   	...
						   	$output .= '<div class="dashboard-box-content zero_padding">';
						   		$output .= '<div class="row row_zero_margin ">';
						   			...
						   			$output .= '<div class="col-sm-4">';
						   				$output .= '<h4>'.__('State Survey','nex-forms').'</h4>';
						   				
						   				$output .= '<div class="report_box_main common_box_main">';
							   				$output .= '<div class="report_box common_box">';
								   				$output .= '<div class="inner_common_box completed_box">';
								   					$output .= '<p>' . get_total_state_submitted_entries() . '</p>';
								   					$output .= '<p>'.__('Submitted','nex-forms').'</p>';
								   				$output .= '</div>';
							   				$output .= '</div>';

							   				$output .= '<div class="report_box common_box">';
							   					$output .= '<div class="inner_common_box pending_box">';
								   					$output .= '<p>' . get_total_state_pending_entries() . '</p>';
								   					$output .= '<p>'.__('Draft','nex-forms').'</p>';
							   					$output .= '</div>';
							   				$output .= '</div>';
						   				$output .= '</div>';
						   			...
						   			$output .= '<div class="col-sm-8">';
						   				$output .= '<h4>'.__('District Survey','nex-forms').'</h4>';

						   				$output .= '<div class="report_box_main common_box_main">';

										   $output .= '<div class="report_box common_box">';
											   $output .= '<div class="inner_common_box draft_box">';
												   $output .= '<p>' . get_total_district_draft_entries() . '</p>';
												   $output .= '<p>'.__('Draft','nex-forms').'</p>';
											   $output .= '</div>';
										   $output .= '</div>';
							   				...
							   				$output .= '<div class="report_box common_box">';
								   				$output .= '<div class="inner_common_box completed_box">';
								   					$output .= '<p>' . get_total_district_submitted_entries() . '</p>';
								   					$output .= '<p>'.__('Submitted','nex-forms').'</p>';
								   				$output .= '</div>';
							   				$output .= '</div>';
							   				...
							   				$output .= '<div class="report_box common_box">';
							   					$output .= '<div class="inner_common_box pending_box">';
								   					$output .= '<p>' . get_total_district_pending_entries() . '</p>';
								   					$output .= '<p>'.__('Pending','nex-forms').'</p>';
							   					$output .= '</div>';
							   				$output .= '</div>';

							   				$output .= '<div class="report_box common_box">';
							   					$output .= '<div class="inner_common_box under_review_box">';
								   					$output .= '<p>' . get_total_district_under_review_entries() . '</p>';
								   					$output .= '<p>'.__('Under Review','nex-forms').'</p>';
							   					$output .= '</div>';
							   				$output .= '</div>';

							   				$output .= '<div class="report_box common_box">';
							   					$output .= '<div class="inner_common_box reviwed_box">';
								   					$output .= '<p>' . get_total_district_reviewed_entries() . '</p>';
								   					$output .= '<p>'.__('Reviewd by State and Submitted','nex-forms').'</p>';
							   					$output .= '</div>';
							   				$output .= '</div>';
						   				$output .= '</div>';
			  	...
			  	$output .= '</div>';
			  	// ======================================================
			  	// == End of "Submission Records"
			  	// ======================================================


			  	// ======================================================
			  	// == Start of "Forms records"
			  	// == Added By Milan Hirapra
			  	// == 08-01-2121
			  	// ======================================================
			  	$allStateQuery = $wpdb->prepare('SELECT * FROM '. STATE_TBL . ' ORDER BY Id ASC');
                ...
                $output .= '<div class="dashboard-block">';

                $output .= ' <table border="0" cellspacing="5" cellpadding="5">
                            <tbody>
                                <tr>
                                    <td><input class="form-control" type="text" class="form-control ml-0 pl-2" id="searchdistrictinput"
                                            placeholder="Search by Username" autofocus/></td>
                                </tr>
                                <tr>
                                    <td>District Submission date:</td>
                                    <td><input class="form-control"  name="min" id="min" type="text" placeholder="From Date"></td>
                                    <td><input class="form-control"  name="max" id="max" type="text" placeholder="To Date"></td>
                                </tr>
                                <tr></tr>
                                <tr>
                                    <td>Final Submission date:</td>
                                     <td><input class="form-control"  name="fmin" id="fmin" type="text" placeholder="From Date"></td>
                                    <td><input class="form-control"  name="fmax" id="fmax" type="text" placeholder="To Date"></td>
                                </tr>
                                <tr>
                                <td>';
                              
                                if( $aStates ):
                                    $output .= '<select class="form-control"  name="astate_filter" id="astate_filter">';
                                        $output .= '<option value=" ">State</option>';
                                        foreach ( $aStates as $st ) {
                        ...
                        $output .= '</td>
                            <td><button  id="districtSearch" type="button" class="btn btn-sm btn-success form-control">
                                ...
                                $output.= '<td><form id="download_state_csv" name="download_state_csv" method="POST">
												...
												<input type="hidden" name="fstate" class="fstate">
												<input type="submit" class="btn btn-sm btn-success form-control" value="Export state CSV" name="state_csv_download">
											</form></td>';

								$output.= '<td><form id="download_city_csv" name="download_city_csv" method="POST">
												<input type="hidden" name="username" class="fusername">
												<input type="hidden" name="dst_sub_from" class="dst_sub_from">
												<input type="hidden" name="dst_sub_to" class="dst_sub_to">
												<input type="hidden" name="final_sub_from" class="final_sub_from">
                    ...
                    $output .= '<div class="table-responsive">';
                        $output .= '<table class="table dashboard-table" id="stateSurvey">';
                            ...
                            if( $users && !empty($users) ){
                            	...
                            	$output .= '<thead>';
                                    $output .= '<tr>';
                                        ...
                                        $output .= '<th>District Submission date</th>';
                                        $output .= '<th>Final Submission date</th>';
                                        $output .= '<th>District Status</th>';
                                        $output .= '<th>Final Status</th>';
                                        $output .= '<th></th>';
                                        $output .= '<th></th>';
                                        $output .= '<th></th>';
                                        $output .= '<th></th>';
                                ...
                                foreach ( $users as $u_key => $u_id ) {
                                    ...
                                    $output .= '<tr>';
                                        $output .= '<td>';                                 
                                            $output .= '<span>' . get_user_state($u_id['ID']) . '</span>';
                                        $output .= '</td>';

                                        $output .= '<td>';
                                            $output .= '<span>' . get_user_city($u_id['ID']) . '</span>';
                                        $output .= '</td>';

                                        $output .= '<td>';
                                            $output .= '<span>' .$userData->data->user_login. '</span>';
                                        $output .= '</td>';
                                        ...
                                        if( $results && !empty($results) && $results['is_published'] == '1' ){
                                        	...
                                        	$output .= '<td>';
                                                $output .= '<span>'.$results['created_at']. '</span>';
                                            $output .= '</td>';
                                            $output .= '<td>';
                                                $output .= '<span>' .$state_date. '</span>';
                                            $output .= '</td>';

                                            $output .= '<td><span>Completed</span></td>';
                                            $output .= '<td><span>' . $state_status . '</span></td>';

                                            $output .= '<td>';
                                            	$output .= '<a href="' . $view_form_link . '" ' . $view_link_target . '><i class="fa fa-eye"></i> View</a>';
                                            $output .= '</td>';

                                            $output .= '<td>';
                                            	$output .= '<a href="' . $edit_form_link . '" ' . $edit_link_target . '><i class="fa fa-pencil"></i> Edit</a>';
                                            $output .= '</td>';

                                            $output .= '<td>';
                                                $output .= '<a href="' . $pdf_form_link . '" ' . $pdf_link_target . '><i class="fa fa-eye"></i> Download</a>';
                                            $output .= '</td>';

											$output .= '<td>';
                                                $output .= '<a href="' . admin_url( 'admin.php?page=nex-forms-dashboard' ) . '&action=unsubmit&record_id=' . $results['record_id'] . '&user_id=' . $results['user_id'] . '&_nonce=' . wp_create_nonce('checklist-unsubmit') . '" ' . $pdf_link_target . '><i class="fa fa-unlock-alt"></i> Un-submit</a>';
                                            $output .= '</td>';
                ...
                $output .= '</div>';
			  	...
			  	// ======================================================
			  	// == End of "Forms records"
			  	// ======================================================

		  	$output .= '</div>';

		  	//LATEST
			$output .= '<div id="csv_reports" class="dashboard_panel" style="display:none">';
				$output .= '<div class="row row_zero_margin ">';
