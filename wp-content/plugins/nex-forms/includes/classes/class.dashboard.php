	...
	class NEXForms_dashboard{
			...
			$output .= '<div class="dashboard-box form_analytics">';
				...
				$output .= '<div class="dashboard-box-header">';
					...
					$output .= '<div class="controls">';
						$output .= '<div class="col-xs-3" style="display:none;">';
							$output .= '<select class="form_control" name="stats_per_form">';
								$output .= '<option value="0" selected>'.__('All States','nex-forms').'</option>';
								...
								$get_forms = 'SELECT * FROM '.$wpdb->prefix.'states WHERE countries_id = "4" ORDER BY state ASC';
								
								$forms = $wpdb->get_results($get_forms);
								
								foreach($forms as $form)
									$output .= '<option value="'.$form->id.'">'.str_replace('\\','',$form->state).'</option>';
						...
						$output .= '<div class="col-xs-2">';
							...
							$month_array = array('0'=>'All','01'=>'January','02'=>'February','03'=>'March','04'=>'April','05'=>'May','06'=>'June','07'=>'July','08'=>'August','09'=>'September','10'=>'October','11'=>'November','12'=>'December');
						...
						$output .= '</div>';
							...
							$output .= '<button class="btn waves-effect waves-light switch_chart" data-chart-type="radar"><i class="fas fa-spider"></i></button>';
		...
		public function print_chart($args=''){
			...
			$current_year = (int)date('Y');
					...
					$month_query1 = '';
					if( $month_selected != 0 ){
						$month_query1 = ' AND Month(date_time)= "'.$month_selected.'"';
					}

					$year_query1 = '';
					if( $current_year != 0 ){
						$year_query1 = ' AND Year(date_time)= "'.$current_year.'"';
					}
					...
					if($form_id)
					{
						...
						$user_pre_sql    = $wpdb->prepare('SELECT DISTINCT ID FROM '. USER_TBL . ' as u LEFT JOIN '. USERMETA_TBL . ' as u1 ON u1.user_id = u.ID LEFT JOIN '. USERMETA_TBL . ' as u2 ON u2.user_id = u.ID WHERE (u1.meta_key = "state" AND u1.meta_value = "' . $form_id . '" ) ORDER BY Id DESC');
                        ...
                        if( $user_ids && !empty($user_ids) ) {
                            $get_entries    = 'SELECT * FROM '.NEX_FORM_ENTRY_TBL.' WHERE user_Id IN (' . implode(",", $user_ids) . ') AND nex_forms_Id > "0" ' . $year_query1 . $month_query1 . ' AND publish = "1" ORDER BY date_time ASC';
                        } else {
                        	$get_entries    = 'SELECT * FROM '.NEX_FORM_ENTRY_TBL.' WHERE  nex_forms_Id > "0" ' . $year_query1 . $month_query1 . ' AND publish = "1" ORDER BY date_time ASC';
                        }

						$form_entries = $wpdb->get_results($get_entries);

					
						$get_views = $wpdb->prepare('SELECT * FROM '.$wpdb->prefix.'wap_nex_forms_views WHERE publish = "0" ' . $year_query1 . $month_query1 . ' ORDER BY time_viewed ASC');
						$form_views = $wpdb->get_results($get_views);
						
						$get_interactions = $wpdb->prepare('SELECT * FROM '.$wpdb->prefix.'wap_nex_forms_stats_interactions WHERE publish = "0" ' . $year_query1 . $month_query1 . ' ORDER BY time_interacted ASC');
						$form_interactions = $wpdb->get_results($get_interactions);
					}
					else
					{
						$get_entries = 'SELECT * FROM '.$wpdb->prefix.'wap_nex_forms_entries WHERE publish = "1" ' . $year_query1 . $month_query1;
						$form_entries = $wpdb->get_results($get_entries);
						
						$get_views = 'SELECT * FROM '.$wpdb->prefix.'wap_nex_forms_views WHERE publish = "0" ' . $year_query1 . $month_query1;
						$form_views = $wpdb->get_results($get_views);
						
						$get_interactions = 'SELECT * FROM '.$wpdb->prefix.'wap_nex_forms_stats_interactions WHERE publish = "0" ' . $year_query1 . $month_query1;
						$form_interactions = $wpdb->get_results($get_interactions);
					}
						...
						foreach($header_params as $key=>$val)
							...
							if(is_array($val))
								...
								if($val['user_func_class'])
									...
									$output .= '<td><a href="' . get_the_permalink(EDIT_STATE_FORM) . '?record_id=' . $record->Id . '&nex_form_id=' . $record->nex_forms_Id . '&user_id=' . $record->user_Id . '" target="__blank">Edit</a></td>';
								else
